...
class Word2VecTrainer:
    ...
    def __init__(self, output_dir: str = 'models'):
        ...
        self.model_params = {
            'vector_size': 200,      # Higher dimensionality for better semantic representation
            'window': 8,            # Larger window for historical texts (less rigid syntax)
            'min_count': 3,          # Conservative threshold for rare words
            'workers': 4,            # Parallel processing
            'epochs': 30,            # More epochs for better convergence
            'alpha': 0.03,          # Learning rate
            'min_alpha': 0.0001,     # Minimum learning rate
            'sg': 1,                 # Skip-gram (better for rare words)
            'hs': 1,                 # Use negative sampling
            'negative': 15,          # More negative samples for better learning
